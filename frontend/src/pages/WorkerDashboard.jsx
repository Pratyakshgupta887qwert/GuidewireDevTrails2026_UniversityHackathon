...
import {
  ...
  ShieldAlert, History, CheckCircle2
} from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col">
        <header className="h-20 bg-[#0B1222]/80 backdrop-blur-md border-b border-slate-700/50 px-8 flex items-center justify-between sticky top-0 z-20">
          <div className="flex items-center gap-4">
            ...
            <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-[0_0_15px_rgba(16,185,129,0.1)]">
              Active Partner
            </span>
          </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex gap-3 items-start p-2 rounded-xl hover:bg-slate-700/30 transition-colors cursor-pointer border border-transparent hover:border-slate-700/50">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] font-medium text-slate-400 mt-0.5 line-clamp-1">Risk engine is monitoring your zone for automated payouts.</p>
                      </div>
                    </div>
                    <div className="flex gap-3 items-start p-2 rounded-xl hover:bg-slate-700/30 transition-colors cursor-pointer border border-transparent hover:border-slate-700/50">
                      ...
                      <div>
                        <p className="text-sm font-bold text-slate-200">Claims Automation Ready</p>
                        <p className="text-[11px] font-medium text-slate-400 mt-0.5 line-clamp-1">Verify Aadhaar and keep an active shield for automated payouts.</p>
                      </div>
                    </div>
                  </div>
                  <button
        ...
        <main className="p-8 space-y-8 max-w-7xl mx-auto w-full">
          {activeTab === 'overview' && (
            <>
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                  <div className="text-center md:text-left">
                    ...
                    <h3 className="text-4xl md:text-5xl font-black mb-4 leading-tight text-slate-100 drop-shadow-md">
                      {isDisrupted ? 'Automatic Claim Processing...' : 'Real-time Income Protection Active'}
                    </h3>
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                <ActiveShieldCard activePolicy={activePolicy} />
                <AadhaarVerificationCard
                  user={user}
                  activePolicy={activePolicy}
                  onVerifyAadhaar={onVerifyAadhaar}
                  onRefreshProtection={onRefreshProtection}
                />
              </div>
                        ...
                        <div className="absolute inset-0 bg-rose-900/10 backdrop-blur-[2px] flex items-center justify-center overflow-hidden">
                          <div className="grid grid-cols-8 gap-10">
                            {[...Array(32)].map((_, index) => (
                              <CloudRain key={index} className="text-rose-500/20 animate-bounce" style={{ animationDelay: `${index * 0.2}s` }} />
                            ))}
                          </div>
                        </div>
                      )}
                ...
                <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 shadow-2xl shadow-black/20 rounded-[32px] p-8 flex flex-col h-full">
                  ...
                  <div className="space-y-8 flex-1">
                    {claims.length > 0 ? (
                      ...
                      <>
                        <LedgerItem label="Heavy Rain Comp." date="Waiting for next event" amount="+₹0" status="STANDBY" type="payout" />
                        <LedgerItem label="Weekly Shield" date={activePolicy ? `${activePolicy.days_remaining} day(s) left` : 'No active shield'} amount={activePolicy ? `-₹${activePolicy.premium}` : '₹0'} status={activePolicy ? 'ACTIVE' : 'INACTIVE'} type="premium" />
                        <LedgerItem label="Auto Claims" date={user.aadhaar_verified ? 'Aadhaar verified' : 'Verification required'} amount="+₹0" status={user.aadhaar_verified ? 'READY' : 'LOCKED'} type="earning" />
                      </>
                    )}
                  </div>
                  <button
                    onClick={() => setActiveTab('payouts')}
                    className="w-full py-4 bg-slate-800 text-slate-300 border border-slate-700 font-bold rounded-2xl mt-8 hover:bg-slate-700/80 hover:text-slate-100 flex items-center justify-center gap-2 transition-colors"
                  >
          ...
          {activeTab === 'payouts' && <PayoutHistory claims={claims} activePolicy={activePolicy} />}
...
const ActiveShieldCard = ({ activePolicy }) => (
  <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 shadow-2xl shadow-black/20 rounded-[32px] p-8">
    <div className="flex items-center justify-between gap-4 mb-6">
      <div>
        <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-400">Active Weekly Shield</p>
        <h4 className="text-2xl font-black text-slate-100 mt-2">
          {activePolicy ? activePolicy.tier_name : 'No active protection'}
        </h4>
      </div>
      <div className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border ${
        activePolicy ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-400'
      }`}>
        {activePolicy ? activePolicy.status : 'inactive'}
      </div>
    </div>

    {activePolicy ? (
      <div className="grid sm:grid-cols-2 gap-4">
        <ShieldMetric label="Premium" value={`₹${activePolicy.premium}/week`} />
        <ShieldMetric label="Coverage" value={`₹${activePolicy.coverage}`} />
        <ShieldMetric label="Risk Level" value={activePolicy.risk_level} />
        <ShieldMetric label="Days Remaining" value={`${activePolicy.days_remaining}`} />
      </div>
    ) : (
      <div className="rounded-[24px] border border-dashed border-slate-700 bg-slate-900/40 px-6 py-8">
        <p className="text-sm font-bold text-slate-300">No active protection</p>
        <p className="text-sm text-slate-400 mt-2">Pick a weekly shield from the Policy Tiers tab to unlock automated income-loss coverage.</p>
      </div>
    )}
  </div>
);

const ShieldMetric = ({ label, value }) => (
...
const AadhaarVerificationCard = ({ user, activePolicy, onVerifyAadhaar, onRefreshProtection }) => {
  ...
  const handleVerify = async (event) => {
    ...
    try {
      await onVerifyAadhaar(aadhaarNumber);
  ...
  return (
    <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 shadow-2xl shadow-black/20 rounded-[32px] p-8">
      <p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-400">Aadhaar Verification</p>
      <h4 className="text-2xl font-black text-slate-100 mt-2">
        {user.aadhaar_verified ? 'Claim access unlocked' : 'Verify Aadhaar to claim payout'}
      </h4>

      {user.aadhaar_verified ? (
        <div className="mt-6 space-y-4">
          <div className="rounded-[24px] border border-emerald-500/20 bg-emerald-500/10 px-5 py-4 flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center shrink-0">
              <CheckCircle2 size={24} />
            </div>
            <div>
              <p className="text-sm font-black text-emerald-400 uppercase tracking-[0.18em]">Verified</p>
              <p className="text-sm font-bold text-emerald-100 mt-1">
                Aadhaar ending in {user.aadhaar_number?.slice(-4) || 'XXXX'} is now linked to your payout account.
              </p>
            </div>
          </div>
          <div className={`rounded-[24px] border px-5 py-4 ${activePolicy ? 'border-blue-500/20 bg-blue-500/10' : 'border-slate-700/50 bg-slate-900/40'}`}>
            <p className={`text-sm font-black uppercase tracking-[0.18em] ${activePolicy ? 'text-blue-400' : 'text-slate-400'}`}>
              Claim Automation
            </p>
            <p className="text-sm text-slate-300 mt-2">
              {activePolicy
                ? 'Your active shield is eligible for automated claim payouts whenever an admin-triggered disruption occurs.'
                : 'Verification is complete. Buy a weekly shield to become eligible for automated disruption payouts.'}
            </p>
          </div>
        </div>
      ) : (
        <form onSubmit={handleVerify} className="mt-6 space-y-4">
          <div>
            <label className="text-xs font-black uppercase tracking-[0.18em] text-slate-500 block mb-2">12-digit Aadhaar Number</label>
            <input
              type="text"
              value={aadhaarNumber}
              onChange={(event) => setAadhaarNumber(event.target.value.replace(/\D/g, '').slice(0, 12))}
              placeholder="123412341234"
              className="w-full bg-slate-900/50 border border-slate-700 rounded-2xl px-4 py-4 text-sm font-bold text-slate-200 focus:outline-none focus:border-blue-500 transition-all placeholder:text-slate-600"
            />
          </div>
          ...
          <div className="rounded-[24px] border border-amber-500/20 bg-amber-500/10 px-5 py-4">
            <p className="text-sm font-black text-amber-400 uppercase tracking-[0.18em]">Claims Locked</p>
            <p className="text-sm text-amber-100 mt-2">Verify Aadhaar to claim payout. Automated claim creation stays blocked until verification is complete.</p>
          </div>
...
const LedgerItem = ({ label, date, amount, status, type }) => (
  <div className="flex justify-between items-start group cursor-pointer">
    <div className="flex gap-4 items-center">
      ...
      <div>
        <p className="text-sm font-bold text-slate-200 group-hover:text-blue-400 transition-colors capitalize">{label}</p>
